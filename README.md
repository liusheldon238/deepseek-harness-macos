...
- Installs the pinned [dsh-preset-catalog v0.1.1](https://github.com/liusheldon238/dsh-preset-catalog/releases/tag/v0.1.1) and [dsh-model-search v0.1.2](https://github.com/liusheldon238/dsh-model-search/releases/tag/v0.1.2) snapshots from `Resources/Plugins` in that order. The catalog provides six searchable domain presets; model search improves composer and `/model` selection while preserving raw model IDs.
- On every launch, verifies that Node matches the host architecture (Apple Silicon uses arm64 only), checks the latest DeepSeek Harness release, and compares installed registry-plugin versions. Registry updates are bounded and non-blocking; a failed target version enters a six-hour retry cooldown while the verified local version starts normally.
